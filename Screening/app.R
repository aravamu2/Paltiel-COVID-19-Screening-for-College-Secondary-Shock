...
sidebar <- dashboardSidebar(
  ...
  sidebarMenu(
    ...
    menuItem("Calculations (1)", tabName = "calculations_1", icon = icon("table")),
...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(
      ...
      column(width = 2,
             ## Secondary Shock
             box(title = "Secondary Shock", width = NULL, solidHeader = TRUE, status = input_element_color,
                 collapsible = TRUE, collapsed = FALSE,
                 radioButtons("secondary_shock", "Secondary shock?", choices = c("Yes", "No"), selected = "No"),
                 numericInput("new_day", "Day of secondary shock", value = 20,
                              min = 0, max = 80),
                 numericInput("new_infections", "Number of new infections", value = 10,
                              min = 0),
                 numericInput("new_R0", "New R0", value = 2.5, min = 0.1, max = 5, step = 0.5),
                 selectizeInput("new_frequency_of_screening", "New frequency of screening",
                                choices = c("Symptoms Only",
                                            "Every 4 weeks",
                                            "Every 3 weeks",
                                            "Every 2 weeks",
    ...
    ## Calculations ------------------------------------------------------------
    tabItem(
      tabName = "calculations_1",
        box(tableOutput("table_1"), width = 1200)
    ),
    tabItem(
      tabName = "calculations_2",
        box(tableOutput("table_2"), width = 1200)
    ),
    ## References --------------------------------------------------------------
    tabItem(
      tabName = "references",
      ...
      p("The methodology for this application comes from the JAMA Network Open 
        ...
        a("online.", 
          href = "https://docs.google.com/spreadsheets/d/1otD4h-DpmAmh4dUAM4favTjbsly3t5z-OXOtFSbF1lY/edit#gid=1783644071"),
        " Our implementation currently reproduced this spreadsheet expands on it."),
      h2("Glossary"),
      ...
      p(strong("Secondary shock:"),"Infections transmitted to students by 
        university employees or members of the surrounding community or during 
        an additional superspreader event, such as move-in."),
...
server <- function(input, output) {
  ...
  observe({
    ...
    showWarningIf(input$new_day < 1, "The value for day of secondary shock you entered is below the recommended minimum of 1")
    showWarningIf(input$new_day > 80, "The value for day of secondary shock you entered is above the recommended maximum of 80.")
    showWarningIf(input$new_infections > 500, "The value for new infections you entered is above the recommended maximum of 500.")
    showWarningIf(input$new_R0 < 0.1, "The value for new R0 you entered is below the recommended minimum of 0.1.")
  ...
  df <- reactive({
    ...
    new.cycles.per.test <- case_when(
      input$new_frequency_of_screening == "Daily" ~ 1*cycles.per.day,
      input$new_frequency_of_screening == "Every 2 days" ~ 2*cycles.per.day,
      input$new_frequency_of_screening == "Every 3 days" ~ 3*cycles.per.day,
      input$new_frequency_of_screening == "Weekly" ~ 7*cycles.per.day,
      input$new_frequency_of_screening == "Every 2 weeks" ~ 14*cycles.per.day,
      input$new_frequency_of_screening == "Every 3 weeks" ~ 21*cycles.per.day,
      input$new_frequency_of_screening == "Every 4 weeks" ~ 28*cycles.per.day,
    ...
    superspreader.infections <- superspreader.event * input$new_infections_per_shock
    if(input$secondary_shock == "Yes") {
      superspreader.event[input$new_day*3+1] <- 1
      superspreader.infections[input$new_day*3+1] <- superspreader.infections[input$new_day*3+1] + input$new_infections
    }
    
    for(i in 2:n.cycle) {
      if(input$secondary_shock == "No" | input$new_day > (i/3)) {
        mat <- 
          rbind(
            mat,
            c(i,
              max(0,mat[i,2]*(1-beta*(mat[i,5]/(mat[i,2]+mat[i,5]+mat[i,4])))+mat[i,3]*mu-mat[i-1,2]*(1-input$test_specificity)/cycles.per.test-superspreader.event[i+1]*superspreader.infections[i+1]),
      ...
      } else if(input$secondary_shock == "Yes" & input$new_day <= (i/3)) {
        mat <- 
          rbind(
            mat,
            c(i,
              ...
              max(0,mat[i,6]*(1-delta-rho)+(mat[i,5]+mat[i,7])*sigma),
              max(0,mat[i,7]*(1-sigma-rho)+mat[i-1,5]*input$test_sensitivity/new.cycles.per.test),
              max(0,mat[i,8]+(mat[i,5]+mat[i,6]+mat[i,7])*rho),
              max(0,delta*mat[i,6]+mat[i,9]))
          )
      }
    ...
    names.df <- c("Cycle","Susceptible","FP","Exposed","Asymptomatic","Symptomatic","TP","Recovered","Dead","Superspreader Event", "Superspreader Infections")
    df <- 
      mat %>% 
      as_tibble() %>% 
      rename_all(~names.df) %>% 
      mutate(`Persons Tested` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+1)/3),(lag(Susceptible,1,NA)+lag(Exposed,1,NA)+lag(Asymptomatic,1,NA))/cycles.per.test,(lag(Susceptible,1,NA)+lag(Exposed,1,NA)+lag(Asymptomatic,1,NA))/new.cycles.per.test),
             `Total TPs` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+2)/3),lag(Asymptomatic,2,NA)*input$test_sensitivity/cycles.per.test,lag(Asymptomatic,2,NA)*input$test_sensitivity/new.cycles.per.test),
             `Total FPs` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+2)/3),lag(Susceptible,2,NA)*(1-input$test_specificity)/cycles.per.test,lag(Susceptible,2,NA)*(1-input$test_specificity)/new.cycles.per.test),
             `Total TNs` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+2)/3),lag(Susceptible,2,NA)*input$test_specificity/cycles.per.test,lag(Susceptible,2,NA)*input$test_specificity/new.cycles.per.test),
             `Total FNs` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+2)/3),lag(Exposed,2,NA)+lag(Asymptomatic,2,NA)*(1-input$test_sensitivity)/cycles.per.test,lag(Exposed,2,NA)+lag(Asymptomatic,2,NA)*(1-input$test_sensitivity)/new.cycles.per.test)) %>% 
      mutate(Day = Cycle/cycles.per.day,
             `True Positive` = TP,
             Symptoms = Symptomatic,
             `False Positive` = FP,
             Total = TP+Symptoms+FP) %>% 
      mutate(`New Infections` = ifelse(input$secondary_shock == "No" | input$new_day > ((Cycle+1)/3),lag(Asymptomatic,1,NA)*beta*lag(Susceptible,1,NA)/(lag(Susceptible,1,NA)+lag(Exposed,1,NA)+lag(Asymptomatic,1,NA)),lag(Asymptomatic,1,NA)*new.beta*lag(Susceptible,1,NA)/(lag(Susceptible,1,NA)+lag(Exposed,1,NA)+lag(Asymptomatic,1,NA))),
  ...
  output$table_1 <- renderTable({
    df() %>% 
      select(`Cycle`:`Superspreader Infections`)
  })
  
  output$table_2 <- renderTable({
    df() %>% 
      select(Day,`Persons Tested`:`%Cumulative Infections`) %>% 
      mutate(`%Cumulative Infections` = `%Cumulative Infections`*100)
  })
